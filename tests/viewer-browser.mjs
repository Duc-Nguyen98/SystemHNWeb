  ...
  for (const engineName of (process.env.QA_BROWSERS || 'chromium').split(',')) {
    ...
    try {
      for (const [width,height,deviceScaleFactor] of viewports) {
        ...
        console.log(`${engineName}: 70 boards + interactions at ${width}x${height} DPR ${deviceScaleFactor}`);
        await context.close();
      }
      ...
      for(const prefix of ['', 'docs/']) {
        ...
        for(const [route,hash,count] of [['index.html','auth-desktop',13],['index.html','auth-tablet',13],['index.html','overview-desktop',22],['index.html','overview-tablet',22],['previews/overview-filter-v2/index.html','desktop',13],['previews/overview-filter-v2/index.html','tablet',13]]) {
